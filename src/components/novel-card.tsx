...
import { sourceLanguageLabel } from "#/lib/novels/novels-core";
    ...
    <article className="border rounded-md border-border p-4 hover:bg-card hover:border-black transition-colors cursor-pointer focus:outline-ring focus-visible:outline-2 focus-visible:outline-ring">
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1 text-sm text-muted-foreground line-clamp-1">
            {sourceLanguageLabel(novel.sourceLanguage)} → English
          </p>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        <div className="text-right">
          <p className="text-sm font-medium text-foreground">{novel.totalChapters} chapters</p>
          <p className="text-xs text-muted-foreground">{novel.createdAt}</p>
        </div>
      </div>
    </article>
  );
